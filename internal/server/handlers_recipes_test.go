...
func TestHandlers_Recipes(t *testing.T) {
	...
	t.Run("user has recipes", func(t *testing.T) {
		...
		assertStringsInHTML(t, got, []string{
			...
			`<img class="h-full min-h-28 w-24 object-cover rounded-t-lg sm:h-40 sm:min-h-0 sm:min-w-full sm:w-full" src="/data/images/Placeholders/placeholder.recipe.webp" alt="Image for the One recipe">`,
			`<img class="h-full min-h-28 w-24 object-cover rounded-t-lg sm:h-40 sm:min-h-0 sm:min-w-full sm:w-full" src="/data/images/Placeholders/placeholder.recipe.webp" alt="Image for the Two recipe">`,
			`<img class="h-full min-h-28 w-24 object-cover rounded-t-lg sm:h-40 sm:min-h-0 sm:min-w-full sm:w-full" src="/data/images/Placeholders/placeholder.recipe.webp" alt="Image for the Three recipe">`,
			`<div class="card-body justify-between px-4 !py-0 sm:!p-4">`,
		})
...
func TestHandlers_Recipes_Highlight(t *testing.T) {
	srv := newServerTest()
	recipe := models.Recipe{ID: 1, Name: "One", Category: "Lunch", Description: "Recipe one"}
	srv.Repository = &mockRepository{RecipesRegistered: map[int64]models.Recipes{1: {recipe}}}
	defer func() {
		srv.Repository = &mockRepository{}
	}()

	rr := sendHxRequestAsLoggedInNoBody(srv, http.MethodPost, "/recipes/1/highlight")

	assertStatus(t, rr.Code, http.StatusOK)
	assertStringsInHTML(t, getBodyHTML(rr), []string{
		`hx-post="/recipes/1/highlight?compact=true"`,
		`shrink-0 p-0 btn-xs size-6 sm:btn-sm sm:size-8`,
		`aria-label="Unpin recipe"`,
		`btn-primary`,
	})

	updated, err := srv.Repository.Recipe(1, 1)
	if err != nil {
		t.Fatalf("expected recipe to be retrievable: %v", err)
	}
...
func TestHandlers_Recipes_Edit(t *testing.T) {
	...
	t.Run("successful request", func(t *testing.T) {
		...
		assertStringsInHTML(t, getBodyHTML(rr), []string{
			...
			`<select name="language" class="select select-bordered select-sm md:w-28 lg:w-40 xl:w-44"><option value="en">English</option><option value="it" selected>Italian</option></select>`,
...
func TestHandlers_Recipes_Scale(t *testing.T) {
	...
	t.Run("valid request double yield", func(t *testing.T) {
		...
		want := []string{
			...
			`<label class="label justify-start"><input type="checkbox" class="checkbox"> <span class="label-text pl-2">Un sac de chips de 2 kg</span>`,
			...
			`<label class="label justify-start"><input type="checkbox" class="checkbox"> <span class="label-text pl-2">Spaghetti grossi 640 g</span>`,
			`<label class="label justify-start"><input type="checkbox" class="checkbox"> <span class="label-text pl-2">Aglio 6 spicchi</span>`,
			`<label class="label justify-start"><input type="checkbox" class="checkbox"> <span class="label-text pl-2">Olio extravergine d'oliva 140 g</span>`,
			`<span>Spaghetti grossi 640 g</span>`,
			`<span>Aglio 6 spicchi</span>`,
			`<span>Olio extravergine d'oliva 140 g</span>`,
			`<div id="print-yield" class="hidden print:grid print:place-items-center print:gap-1 print:text-center" hx-swap-oob="outerHTML:#print-yield"><div class="hidden print:block print:h-6">&nbsp;</div><div class="font-semibold">8</div><div class="text-xs uppercase tracking-wide">Servings</div></div>`,
		}
		assertStringsInHTML(t, getBodyHTML(rr), want)
	})
}
...
func TestHandlers_Recipes_Search(t *testing.T) {
	...
	t.Run("full page preserves advanced query text", func(t *testing.T) {
		rr := sendRequestAsLoggedInNoBody(srv, http.MethodGet, uri+"?q=name%3ALovely%20Canada")

		assertStatus(t, rr.Code, http.StatusOK)
		assertStringsInHTML(t, getBodyHTML(rr), []string{`id="search_recipes"`, `value="name:Lovely Canada"`})
	})
	...
	searches := []struct {
	...
	}{
		{
			...
			want: []string{
				`>Lovely Canada</h2>`,
				`hx-post="/recipes/2/highlight?compact=true"`,
				`>Lovely Ukraine</h2>`,
				`hx-post="/recipes/3/highlight?compact=true"`,
			},
		},
		{
			query: "chi",
...
func TestHandlers_Recipes_Share(t *testing.T) {
	...
	t.Run("access share link anonymous", func(t *testing.T) {
		...
		assertStringsInHTML(t, body, []string{
			...
			`<span>Ing2</span>`,
			`<span>Ing3</span>`,
			`<div class="hidden col-span-5 overflow-visible print:block print:col-span-1 print:m-0 print:bg-white print:p-3 print:rounded-md print:break-inside-avoid print:h-full"><h1 class="text-sm font-bold print:mb-1 print:border-b print:border-black print:pb-1 print:text-center">Instructions</h1>`,
			`<li class="print:mr-4 print:mb-1 print:pb-1 print:border-b print:border-gray-300 print:break-inside-avoid"><span class="text-sm whitespace-pre-line">Ins1</span></li>`,
...
func TestHandlers_Recipes_View(t *testing.T) {
	...
	for _, tc := range testcases {
		t.Run("recipe is in user's collection when "+tc.name, func(t *testing.T) {
			...
			assertStringsInHTML(t, getBodyHTML(rr), []string{
				...
				`<img id="output" style="object-fit: cover" alt="Image of the recipe" class="w-full max-h-80 md:max-h-[34rem]" src="/data/images/Placeholders/placeholder.recipe.webp">`,
				`<div class="badge badge-primary badge-outline">American</div>`,
				`<p class="text-center">Language:<br class="hidden md:block">Italian</p>`,
	...
	testcases2 := []struct {
		...
		},
		{
			...
			videos: nil,
			want: []string{
				`<div class="hidden print:flex print:justify-center print:mt-0 print:mb-1 print:break-inside-avoid"><img style="object-fit: cover" alt="Image of the recipe" class="print:h-64 print:w-full print:max-w-full" src="/data/images/` + anImage1.String() + `.webp"></div>`,
