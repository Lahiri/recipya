...
func TestHandlers_Settings(t *testing.T) {
	...
	t.Run("demo sees fake connections data", func(t *testing.T) {
		...
		assertStringsNotInHTML(t, getBodyHTML(rr), []string{
			`<input name="email.from" type="text" placeholder="SMTP email" value="demo@demo.com" autocomplete="off" class="input input-bordered input-sm w-full">`,
...
func TestHandlers_Settings_Config(t *testing.T) {
	...
	fields := "email.from=test%40gmail.com&email.host=smtp.gmail.com&email.username=my%40domain.com&email.password=app%20password&integrations.ocr.key=JGKL&integrations.ocr.url=https%3A%2F%2Fwww.google.com&server.autologin=on&server.noSignups=on&server.production=on"
